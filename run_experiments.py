...
def run_experiment(run_id:int, ea_id:int, problem_id:int, seed:int, target_evals:int, options:dict) -> None:
    ...
    logger.info(f'{timestamp} | {process} | Begin the run: {run_id} with {ea_data}')
    ...
    try:
        ...
        util.write_to_csv(Path('archived_final_pops/' + str(run_id) + '.csv'), final_pop)
        ...
        file = Path('archived_final_pops/' + str(run_id) + '_error.txt')
